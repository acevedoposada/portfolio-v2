...
function Tab({ label, selected, onChange, onClick, value, badge }: TabProps): JSX.Element {
  ...
  const selectedClass = useMemo(
    () =>
      cn(selected ? "opacity-100" : "opacity-50", "transition-opacity duration-500 ease-in-out"),
    [selected]
  );

  return (
    <button className={cn(styles.tab, selected && styles["tab--selected"])} onClick={handleClick}>
      {label}
      {badge && (
        <Chip
