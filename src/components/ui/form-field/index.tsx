...
function FormField({
  ...
  const validateIcon = useCallback((IconValidate?: FormFieldIcon) => {
    ...
    if (typeof IconValidate === "string") {
      return (
        <i
          className={cn(`ti ti-${IconValidate}`, styles["form-field__btn__icon"])}
          aria-hidden="true"
        />
      );
    }
  ...
  const formField = (
    ...
    >
      {prefix && <span className={cn(styles["form-field__prefix"], classes.prefix)}>{prefix}</span>}
      <div className={cn(styles["form-field__input"], classes.input)}>
        {iconValidated && iconPosition === IconPosition.left && (
          <span className={styles["form-field__input__icon"]}>{iconValidated}</span>
        )}
      ...
      {suffix && <span className={cn(styles["form-field__suffix"], classes.suffix)}>{suffix}</span>}
      {button && (
        <button
  ...
  return useMemo(
    () => (label || helpText ? fieldContainer : formField),
